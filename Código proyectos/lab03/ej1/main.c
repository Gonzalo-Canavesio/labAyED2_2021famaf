...
int main(int argc, char *argv[]) {
    ...
    /* funciones programadas en la parte B */

    printf("La menor temperatura minima fue %d\n", temp_min(array));
    int maxs_temps_a[YEARS];
    month_t max_m_pres_a[YEARS];    
    mayor_temps_max(array,maxs_temps_a);
    mes_max_prec(array,max_m_pres_a);

    for(int a=0; a<YEARS;++a){
        printf("La mayor temperatura maxima de %d fue %d\n", a+FST_YEAR, maxs_temps_a[a]);
    };
    for(int a=0; a<YEARS;++a){
        printf("La mayor cantidad de precipitaciones de %d fue en el mes numero %d\n", a+FST_YEAR, max_m_pres_a[a]+1);
    };     
